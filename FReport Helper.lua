...
local script_url = "https://github.com/nikeveryone/Scripts-by-N.Everyone/blob/main/FReport%20Helper.luac?raw=true" -- тут свою ссылку
...
local gps7 = "[1] СТО номер: 1\n[2] СТО номер: 2\n[3] СТО номер: 3"
local gps8 = "[1] Служба такси №1\n[2] Служба такси №2\n[3] Служба такси №3"
local gps9 = "[1] Транспортная компания №1\n[2] Транспортная компания №2\n[3] Транспортная компания №3"
...
local gps10 = "[1] Автосалон [LS] - Nope\n[2] Автосалон [SF] - C\n[3] Автосалон [SF] - B\n[4] Автосалон [LV] - A\n[5] Автосалон [LV] - Real Cars\n[6] Мотосалон\n[7] Яхт-клуб\n[8] Аэроклуб\n[9] Приобретение страховки и номеров\n[10] Парковки"
local gps12 = "[1] Центр развлечений\n[2] Casino Four Dragons\n[3] Casino Caligula"
local gps13 = "[1] Мой дом\n[2] Мой бизнес\n[3] Мой транспорт\n[4] Дом семьи"
...
function main()
  ...
  imgui.Process = true
	...
	notify.addNotify("{ffdead}FReport Helper:\n", "Samp Umbrella Project © 2022", 2, 2, 6)
	notify.addNotify("{ffdead}FReport Helper:\n", "Скрипт успешно загружен.", 2, 2, 6)
	sampAddChatMessage('{ffdead}[FReport Helper]: {FFFFFF}Скрипт загрzdbzbDzddbfbzужен. |{f44f36}V1.0{FFFFFF}| | By {32CD32}N.Everyone {FFFFFF}|', 0xffdead)
  ...
  thread = lua_thread.create_suspended(thread_function)
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			updateIni =inicfg.load(nil, update_path)
				...
				sampAddChatMessage("Кароче есть обнова, версия вота - " .. updateIni.info.vers_text, -1)
				update_state = true
				end
	...
	if status == dlstatus.STATUSEX_ENDDOWNLOAD then
		sampAddChatMessage("Скрипт обновился ёпта", -1)
		thisScript()reload:()
		end
	end)
	break
...
function sampev.onServerMessage(color, text)
	if text:find("%[REPORT%]") or text:find("%[H%] %[ASK%]") then
...
function imgui.OnDrawFrame()
	...
	if admmenu.v then
		...
		local btn_size = imgui.ImVec2(-0.1, 30)
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(723, 390), imgui.Cond.FirstUseEver)
		imgui.Begin(u8"FReport Helper| Главное меню скрипта", admmenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		imgui.BeginChild("leAH", imgui.ImVec2(250, 345), true)
		...
		imgui.Separator()
		imgui.Text(u8"\n")
		imgui.CenterTextColoredRGB('Scripts with love from N.Everyone', btn_siz)
		imgui.Text(u8"\n")
		imgui.Separator()
		imgui.CenterTextColoredRGB('S', btn_siz)
		imgui.CenterTextColoredRGB('U', btn_siz)
		imgui.CenterTextColoredRGB('P', btn_siz)
		imgui.CenterTextColoredRGB('©', btn_siz)
		imgui.CenterTextColoredRGB('2022', btn_siz)
		imgui.Separator()
		...
		imgui.EndChild()
		imgui.SameLine()
		imgui.BeginChild("right", imgui.ImVec2(0, 345), true)
		if selected == 0 then
			imgui.CenterTextColoredRGB('Текущая версия: {f44f36}1.0', btn_size)
			imgui.Separator()
			imgui.CenterTextColoredRGB('Разработчики скрипта: {32CD32}Nik_Everyone {ffffff}, {ffdead}Chad_Deep', btn_size)
			imgui.Separator()
			...
			imgui.CenterTextColoredRGB('для Администрации {ffdead}Flin RP{ffffff} в сфере репортов.', btn_siz)
			imgui.NewLine()
			imgui.CenterTextColoredRGB('Всю информацию и функции скрипта сможете узнать в разделе:', btn_siz)
			imgui.CenterTextColoredRGB('                 Информация и функции скрипта.             ', btn_siz)
			imgui.CenterTextColoredRGB('По всем вопросам и проблемам обращайтесь в...', btn_siz)
			imgui.CenterTextColoredRGB('...VK разработчика N.Everyone.', btn_siz)
			imgui.NewLine()
			imgui.Separator()
			imgui.CenterTextColoredRGB('ВКонтакте разработчиков скрипта:', btn_size)
			imgui.Separator()
			imgui.NewLine()
			...
			imgui.SameLine()
			if imgui.Button('Chad Deep | Flin-RP 02') then
				os.execute("explorer \"http://vk.com/markowalker\"")
			end
			imgui.NewLine()
			imgui.Separator()
		...
		imgui.EndChild()
		imgui.Text(u8'Nik Everyone  FReport Helper  Все права защищены. Любое копирование прав запрещено.  Samp Umbrella Project © 2022 ')
		imgui.End()
	end
    ...
    if not main_window_state.v and not prices_window_state.v and not gps_window_state.v then
      ...
      imgui.Begin(u8"Выпустить с КПЗ", unjail_menu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBringToFrontOnFocus+ imgui.WindowFlags.NoSavedSettings)
			...
			imgui.CenterTextColoredRGB('S U P © 2022', statew)
			imgui.End()
		end
		...
		if statew.v then
			...
			imgui.SetNextWindowSize(imgui.ImVec2(185, 388), imgui.Cond.FirstUseEver)
		...
		if statew4.v then
			...
			local btn_size = imgui.ImVec2(-0.1, 0)
			local btn_ssize = imgui.ImVec2(-0.3, 40)
			imgui.SetNextWindowSize(imgui.ImVec2(160, 343), imgui.Cond.FirstUseEver)
			...
			if imgui.Button(u8'Оск. Администрации', btn_ssize) then
				sampSendChat('/ban '..other.recon_id..' 60 Оскорбление Администрации.')
			end
			imgui.CenterTextColoredRGB('S U P © 2022', statew)
			imgui.End()
		end
		if statew5.v then
			...
			local btn_size = imgui.ImVec2(-0.1, 0)
			local btn_ssize = imgui.ImVec2(-0.3, 40)
			imgui.SetNextWindowSize(imgui.ImVec2(205, 388), imgui.Cond.FirstUseEver)
		...
		if statew9.v then
			...
			local btn_size = imgui.ImVec2(-0.1, 0)
			local btn_ssize = imgui.ImVec2(-0.3, 40)
			imgui.SetNextWindowSize(imgui.ImVec2(160, 388), imgui.Cond.FirstUseEver)
		...
		if statew6.v then
			...
			local btn_size = imgui.ImVec2(-0.1, 0)
			local btn_ssize = imgui.ImVec2(-0.3, 40)
			imgui.SetNextWindowSize(imgui.ImVec2(150, 388), imgui.Cond.FirstUseEver)
			imgui.SetNextWindowPos(imgui.ImVec2(iScreenWidth / 2, iScreenHeight / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(-1.5, 0.5))
    ...
    if main_window_state.v then
        imgui.SetNextWindowSize(imgui.ImVec2(485, 410), imgui.Cond.FirstUseEver)
        imgui.SetNextWindowPos(imgui.ImVec2(main_x / 2, main_y / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        ...
        imgui.Text(u8"Жалоба от "..nick_report.." ["..id_report.."]")
				imgui.SameLine()
				if imgui.Button(u8'Разморозить', imgui.ImVec2(110, 20)) then
					main_window_state.v = false
					prices_window_state.v = false
					gps_window_state.v = false
					lua_thread.create(function()
				...
				imgui.SameLine()
			if imgui.Button(u8'Заспавнить', imgui.ImVec2(110, 20)) then
					main_window_state.v = false
					prices_window_state.v = false
					gps_window_state.v = false
					lua_thread.create(function()
        ...
        if imgui.Button(u8'Не чиним', imgui.ImVec2(150, 22)) then
        	...
        	lua_thread.create(function()
	        	...
	        	sampAddChatMessage(tag.."{FFFFFF} Вы телепортировали авторва репорта к себе.", 0xFFFFFF)
	        end)
				end
		...
		imgui.Separator()
		imgui.Text(u8"\n")
		imgui.CenterTextColoredRGB('Nik Everyone                     Samp Umbrella Project © 2022                FReport Helper', btn_siz)
    ...
    if prices_window_state.v then
		...
		imgui.CenterTextColoredRGB('S U P © 2022', btn_siz)
        imgui.End()
    end
    if gps_window_state.v then
    	...
    	if imgui.CollapsingHeader(u8"[7] СТО") then
    		imgui.Text(u8(gps7))
    	end
